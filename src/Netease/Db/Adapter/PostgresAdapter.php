...
class PostgresAdapter extends PdoAdapter
{
    ...
    protected static $specificColumnTypes = [
        ...
        self::Netease_TYPE_JSONB,
        self::Netease_TYPE_CIDR,
        self::Netease_TYPE_INET,
        self::Netease_TYPE_MACADDR,
        self::Netease_TYPE_INTERVAL,
    ...
    public function connect()
    {
        if ($this->connection === null) {
            if (!class_exists('PDO') || !in_array('pgsql', PDO::getAvailableDrivers(), true)) {
                ...
                throw new RuntimeException('You need to enable the PDO_Pgsql extension for Netease to run properly.');
    ...
    public function getColumns($tableName)
    {
        ...
        foreach ($columnsInfo as $columnInfo) {
            ...
            if ($isUserDefined) {
                ...
                $columnType = $this->getNeteaseType($columnInfo['data_type']);
            }
            ...
            if (in_array($columnType, [static::Netease_TYPE_TIME, static::Netease_TYPE_DATETIME], true)) {
                $column->setPrecision($columnInfo['datetime_precision']);
            } elseif (
                !in_array($columnType, [
                    self::Netease_TYPE_SMALL_INTEGER,
                    self::Netease_TYPE_INTEGER,
                    self::Netease_TYPE_BIG_INTEGER,
                ], true)
            ) {
    ...
    public function getSqlType($type, $limit = null)
    {
        switch ($type) {
            ...
            case static::Netease_TYPE_JSONB:
            case static::Netease_TYPE_UUID:
            case static::Netease_TYPE_CIDR:
            case static::Netease_TYPE_INET:
            case static::Netease_TYPE_MACADDR:
            ...
            case static::Netease_TYPE_BINARY:
                return ['name' => 'bytea'];
            case static::Netease_TYPE_INTERVAL:
     ...
     * @throws \Netease\Db\Adapter\UnsupportedColumnTypeException
     * @return string Netease type
     */
    public function getNeteaseType($sqlType)
    {
        switch ($sqlType) {
            case 'character varying':
            case 'varchar':
                return static::Netease_TYPE_STRING;
            case 'character':
            case 'char':
                return static::Netease_TYPE_CHAR;
            case 'text':
                return static::Netease_TYPE_TEXT;
            case 'json':
                return static::Netease_TYPE_JSON;
            case 'jsonb':
                return static::Netease_TYPE_JSONB;
            case 'smallint':
                return static::Netease_TYPE_SMALL_INTEGER;
            case 'int':
            case 'int4':
            case 'integer':
                return static::Netease_TYPE_INTEGER;
            case 'decimal':
            case 'numeric':
                return static::Netease_TYPE_DECIMAL;
            case 'bigint':
            case 'int8':
                return static::Netease_TYPE_BIG_INTEGER;
            case 'real':
            case 'float4':
                return static::Netease_TYPE_FLOAT;
            case 'double precision':
                return static::Netease_TYPE_DOUBLE;
            case 'bytea':
                return static::Netease_TYPE_BINARY;
            case 'interval':
                return static::Netease_TYPE_INTERVAL;
            case 'time':
            case 'timetz':
            case 'time with time zone':
            case 'time without time zone':
                return static::Netease_TYPE_TIME;
            case 'date':
                return static::Netease_TYPE_DATE;
            case 'timestamp':
            case 'timestamptz':
            case 'timestamp with time zone':
            case 'timestamp without time zone':
                return static::Netease_TYPE_DATETIME;
            case 'bool':
            case 'boolean':
                return static::Netease_TYPE_BOOLEAN;
            case 'uuid':
                return static::Netease_TYPE_UUID;
            case 'cidr':
                return static::Netease_TYPE_CIDR;
            case 'inet':
                return static::Netease_TYPE_INET;
            case 'macaddr':
                return static::Netease_TYPE_MACADDR;
            default:
    ...
    protected function getDefaultValueDefinition($default, $columnType = null)
    {
        ...
        } elseif ($columnType === static::Netease_TYPE_BOOLEAN) {
    ...
    protected function getColumnSqlDefinition(Column $column)
    {
        ...
        if ($column->isIdentity()) {
            ...
            if ($sqlType['name'] === static::Netease_TYPE_DECIMAL && ($column->getPrecision() || $column->getScale())) {
                $buffer[] = sprintf(
                    '(%s, %s)',
                    $column->getPrecision() ?: $sqlType['precision'],
                    $column->getScale() ?: $sqlType['scale']
                );
            } elseif ($sqlType['name'] === self::Netease_TYPE_GEOMETRY) {
            ...
            } elseif (in_array($sqlType['name'], [self::Netease_TYPE_TIME, self::Netease_TYPE_TIMESTAMP], true)) {
                ...
                !in_array($column->getType(), [
                    self::Netease_TYPE_TINY_INTEGER,
                    self::Netease_TYPE_SMALL_INTEGER,
                    self::Netease_TYPE_INTEGER,
                    self::Netease_TYPE_BIG_INTEGER,
                    self::Netease_TYPE_BOOLEAN,
                    self::Netease_TYPE_TEXT,
                    self::Netease_TYPE_BINARY,
                ], true)
            ) {
