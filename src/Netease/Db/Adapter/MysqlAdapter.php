...
use Netease\Db\Util\AlterInstructions;
use Netease\Util\Literal;
use RuntimeException;

/**
 * Netease MySQL Adapter.
 *
...
class MysqlAdapter extends PdoAdapter
{
    ...
    protected static $specificColumnTypes = [
        self::Netease_TYPE_ENUM,
        self::Netease_TYPE_SET,
        self::Netease_TYPE_YEAR,
        self::Netease_TYPE_JSON,
        self::Netease_TYPE_BINARYUUID,
        self::Netease_TYPE_TINYBLOB,
        self::Netease_TYPE_MEDIUMBLOB,
        self::Netease_TYPE_LONGBLOB,
    ];
    ...
    protected $signedColumnTypes = [
        self::Netease_TYPE_INTEGER => true,
        self::Netease_TYPE_TINY_INTEGER => true,
        self::Netease_TYPE_SMALL_INTEGER => true,
        self::Netease_TYPE_BIG_INTEGER => true,
        self::Netease_TYPE_FLOAT => true,
        self::Netease_TYPE_DECIMAL => true,
        self::Netease_TYPE_DOUBLE => true,
        self::Netease_TYPE_BOOLEAN => true,
    ];
    ...
    public function connect()
    {
        if ($this->connection === null) {
            if (!class_exists('PDO') || !in_array('mysql', PDO::getAvailableDrivers(), true)) {
                ...
                throw new RuntimeException('You need to enable the PDO_Mysql extension for Netease to run properly.');
    ...
    public function getColumns($tableName)
    {
        ...
        foreach ($rows as $columnInfo) {
            $NeteaseType = $this->getNeteaseType($columnInfo['Type']);

            $column = new Column();
            $column->setName($columnInfo['Field'])
                   ->setNull($columnInfo['Null'] !== 'NO')
                   ->setDefault($columnInfo['Default'])
                   ->setType($NeteaseType['name'])
                   ->setSigned(strpos($columnInfo['Type'], 'unsigned') === false)
                   ->setLimit($NeteaseType['limit'])
                   ->setScale($NeteaseType['scale']);

            if ($columnInfo['Extra'] === 'auto_increment') {
                $column->setIdentity(true);
            }

            if (isset($NeteaseType['values'])) {
                $column->setValues($NeteaseType['values']);
            }
     ...
     *
     * @param \Netease\Db\Table\Column $column The column being altered.
     ...
     * {@inheritDoc}
     *
     * @throws \Netease\Db\Adapter\UnsupportedColumnTypeException
     */
    public function getSqlType($type, $limit = null)
    {
        switch ($type) {
            case static::Netease_TYPE_FLOAT:
            case static::Netease_TYPE_DOUBLE:
            case static::Netease_TYPE_DECIMAL:
            case static::Netease_TYPE_DATE:
            case static::Netease_TYPE_ENUM:
            case static::Netease_TYPE_SET:
            case static::Netease_TYPE_JSON:
            // Geospatial database types
            case static::Netease_TYPE_GEOMETRY:
            case static::Netease_TYPE_POINT:
            case static::Netease_TYPE_LINESTRING:
            case static::Netease_TYPE_POLYGON:
                return ['name' => $type];
            case static::Netease_TYPE_DATETIME:
            case static::Netease_TYPE_TIMESTAMP:
            case static::Netease_TYPE_TIME:
                return ['name' => $type, 'limit' => $limit];
            case static::Netease_TYPE_STRING:
                return ['name' => 'varchar', 'limit' => $limit ?: 255];
            case static::Netease_TYPE_CHAR:
                return ['name' => 'char', 'limit' => $limit ?: 255];
            case static::Netease_TYPE_TEXT:
                if ($limit) {
                    $sizes = [
                ...
                return ['name' => 'text'];
            case static::Netease_TYPE_BINARY:
                if ($limit === null) {
                    $limit = 255;
                }

                if ($limit > 255) {
                    return $this->getSqlType(static::Netease_TYPE_BLOB, $limit);
                }

                return ['name' => 'binary', 'limit' => $limit];
            case static::Netease_TYPE_BINARYUUID:
                return ['name' => 'binary', 'limit' => 16];
            case static::Netease_TYPE_VARBINARY:
                if ($limit === null) {
                    $limit = 255;
                }
            ...
            case static::Netease_TYPE_BLOB:
                ...
                return ['name' => 'blob'];
            case static::Netease_TYPE_TINYBLOB:
                // Automatically reprocess blob type to ensure that correct blob subtype is selected given provided limit
                return $this->getSqlType(static::Netease_TYPE_BLOB, $limit ?: static::BLOB_TINY);
            case static::Netease_TYPE_MEDIUMBLOB:
                // Automatically reprocess blob type to ensure that correct blob subtype is selected given provided limit
                return $this->getSqlType(static::Netease_TYPE_BLOB, $limit ?: static::BLOB_MEDIUM);
            case static::Netease_TYPE_LONGBLOB:
                // Automatically reprocess blob type to ensure that correct blob subtype is selected given provided limit
                return $this->getSqlType(static::Netease_TYPE_BLOB, $limit ?: static::BLOB_LONG);
            case static::Netease_TYPE_BIT:
                return ['name' => 'bit', 'limit' => $limit ?: 64];
            case static::Netease_TYPE_SMALL_INTEGER:
                return ['name' => 'smallint', 'limit' => $limit ?: 6];
            case static::Netease_TYPE_TINY_INTEGER:
                return ['name' => 'tinyint', 'limit' => $limit ?: 4];
            case static::Netease_TYPE_INTEGER:
            ...
            case static::Netease_TYPE_BIG_INTEGER:
                return ['name' => 'bigint', 'limit' => $limit ?: 20];
            case static::Netease_TYPE_BOOLEAN:
                return ['name' => 'tinyint', 'limit' => 1];
            case static::Netease_TYPE_UUID:
                return ['name' => 'char', 'limit' => 36];
            case static::Netease_TYPE_YEAR:
    ...
    /**
     * Returns Netease type by SQL type
     *
     * @internal param string $sqlType SQL type
     * @param string $sqlTypeDef SQL Type definition
     * @throws \Netease\Db\Adapter\UnsupportedColumnTypeException
     * @return array Netease type
     */
    public function getNeteaseType($sqlTypeDef)
    {
        $matches = [];
        ...
        if ($type === 'tinyint' && $limit === 1) {
            $type = static::Netease_TYPE_BOOLEAN;
            $limit = null;
        }
        switch ($type) {
            case 'varchar':
                $type = static::Netease_TYPE_STRING;
                if ($limit === 255) {
                    $limit = null;
                }
                break;
            case 'char':
                $type = static::Netease_TYPE_CHAR;
                if ($limit === 255) {
                    $limit = null;
                }
                if ($limit === 36) {
                    $type = static::Netease_TYPE_UUID;
                }
                break;
            case 'tinyint':
                $type = static::Netease_TYPE_TINY_INTEGER;
                $limit = static::INT_TINY;
                break;
            case 'smallint':
                $type = static::Netease_TYPE_SMALL_INTEGER;
                $limit = static::INT_SMALL;
                break;
            case 'mediumint':
                $type = static::Netease_TYPE_INTEGER;
            ...
            case 'bigint':
                ...
                $type = static::Netease_TYPE_BIG_INTEGER;
                break;
            case 'bit':
                $type = static::Netease_TYPE_BIT;
                if ($limit === 64) {
                    $limit = null;
                }
                break;
            case 'blob':
                $type = static::Netease_TYPE_BLOB;
                $limit = static::BLOB_REGULAR;
                break;
            case 'tinyblob':
                $type = static::Netease_TYPE_TINYBLOB;
                $limit = static::BLOB_TINY;
                break;
            case 'mediumblob':
                $type = static::Netease_TYPE_MEDIUMBLOB;
                $limit = static::BLOB_MEDIUM;
                break;
            case 'longblob':
                $type = static::Netease_TYPE_LONGBLOB;
                $limit = static::BLOB_LONG;
                break;
            case 'tinytext':
                $type = static::Netease_TYPE_TEXT;
            ...
            case 'binary':
                ...
                if ($limit === 16) {
                    $type = static::Netease_TYPE_BINARYUUID;
                }
                break;
        }
        ...
        if ($type === static::Netease_TYPE_ENUM || $type === static::Netease_TYPE_SET) {
     ...
     *
     * @param \Netease\Db\Table\Column $column Column
     * @return string
     */
    protected function getColumnSqlDefinition(Column $column)
    {
        ...
        if (
            ...
            && in_array($column->getType(), [
                static::Netease_TYPE_GEOMETRY,
                static::Netease_TYPE_POINT,
                static::Netease_TYPE_LINESTRING,
                static::Netease_TYPE_POLYGON,
            ])
     ...
     *
     * @param \Netease\Db\Table\Index $index Index
     * @return string
     */
     ...
     *
     * @param \Netease\Db\Table\ForeignKey $foreignKey Foreign key
     * @return string
     */
