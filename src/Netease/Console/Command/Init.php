...
{
    protected const FILE_NAME = 'Netease';
    ...
    {
        $this->setDescription('Initialize the application for Netease')
            ->addOption(
                '--format',
                '-f',
            ...
            ->addArgument('path', InputArgument::OPTIONAL, 'Which path should we initialize for Netease?')
            ->setHelp(sprintf(
                '%sInitializes the application for Netease%s',
                PHP_EOL,
                PHP_EOL
            ));
    }
    ...
    /**
     * Return valid $path for Netease's config file.
     *
     * @param \Symfony\Component\Console\Input\InputInterface $input Interface implemented by all input classes.
    ...
    /**
     * Writes Netease's config in provided $path
     *
